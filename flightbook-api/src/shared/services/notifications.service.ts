...
export class NotificationsService {
    ...
    async sendFlightValidationRejected(flight: Flight) {
        ...
        const body = i18n.t('notification.flightValidation.rejected.body', {
            lang: flight.validation.school.language,
            args: {
                date: moment(flight.date).format('DD.MM.YYYY')
            }
        });

        const title = i18n.t('notification.flightValidation.rejected.title', {
            lang: flight.validation.school.language
        });
        const multicastMessage: MulticastMessage = {
            notification: {
